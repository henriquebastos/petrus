...
Worker completion or handler preparation. A seventh loses the accepted
projection commit's acknowledgement and proves fresh load preserves convergence
without another projection. An eighth commits the canonical reset, refuses the
separate real cancellation-tombstone transaction, then proves fresh-load repair
and stale-Worker fencing. A ninth loses the acknowledgement after that
tombstone transaction commits, then proves fresh load recognizes the existing
tombstone without a second custody mutation and again fences the stale Worker.
They do not simulate or claim PostgreSQL power loss, transport, or
