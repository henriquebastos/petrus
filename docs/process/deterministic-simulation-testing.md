...
| `scope_fenced` | Close/reset semantic facts commit before best-effort Dispatch cancellation. | Reload/reconcile repairs pending cancellation with the exact instruction; late terminal delivery is quarantined rather than projected across the fence. | [E: retained [`lifecycle-cancellation-refusal-world-v3.json`](../../tests/dst/fixtures/lifecycle-cancellation-refusal-world-v3.json), `test_close_commits_before_retiring_pending_dispatch_and_late_terminal_is_quarantined`, and `test_restart_repairs_a_committed_cancellation_instruction_idempotently` in [`test_engine.py`](../../tests/petrus/engine/test_engine.py)] |
...
The retained joined-begin commit-refusal artifact deliberately occupies both
columns without merging their claims: the World supplies deterministic fault,
generation, checker, and replay mechanics, while the exact profile runs the
public Absurd Engine provider against disposable PostgreSQL. It qualifies only
the pinned begin+spawn commit-or-vanish boundary and fresh-load repair. It does
not simulate or claim PostgreSQL power loss, Worker execution, transport, or
