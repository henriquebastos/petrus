...
| --- | --- | --- | --- |
| `deliver` | `Engine.deliver` | source path, payload, required stable source identity | accepted, idempotent, refused, acknowledged |
...
| Delivery identity | stable first acceptance, accepted-unfinished exact redelivery, ended exact redelivery, conflicting redelivery |
...
again fences the stale Worker. A fourteenth refuses an identified source-acceptance
...
reconstructs the same unfinished occurrence, completes only that occurrence,
and then returns the prior acknowledgement on subsequent exact redelivery. A
sixteenth refuses canonical `ScopeClosed`
