...
and repair of a refused post-reset cancellation-tombstone transaction.
...
| History append and transaction outcome | Controlled fault at an owned seam | The scenario can refuse or crash around `HistoryStore.append`/`extend` and joined Engine transactions. The production store decides what committed; reload is the oracle for durable truth. The retained joined profiles wrap only the public provider connection's commit acknowledgement, task-spawn call, or post-reset cancellation commit, then use independent PostgreSQL reads to prove the accepted or rolled-back History and Dispatch custody [E: [`test_joined_world.py`](../../tests/dst/test_joined_world.py), [`test_history_backends.py`](../../tests/petrus/impetus/history_store/test_history_backends.py), [`test_engine.py`](../../tests/petrus/engine/test_engine.py)]. |
...
then proves fresh-load repair and stale-Worker fencing. They do not simulate or
claim PostgreSQL power loss, transport, or provider-wide fidelity.
