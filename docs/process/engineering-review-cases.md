# 1 Engineering review cases

These 89 numbered conventions retain the review evidence from which Petrus's
implementation practice developed. Start with the
[engineering guide](engineering-conventions.md) and follow its topic map to
the relevant cases. Ratified reusable rules remain part of that contract;
case 60 remains explicitly provisional.

Examples describe the code and terminology at the time of each review. They
can mention earlier APIs, removed files, or historical commit identifiers
that do not resolve in this repository. Those references locate the original
review context; they are not required checkout or setup steps.
Use current specifications and the glossary for today's contract. A later
ratified decision overrides an older generic rule; surface any conflict.

The review text below is preserved from the former combined guide. New reviews
should add only a reusable rule or a needed correction, with case-specific
narrative retained in its owning work record.
