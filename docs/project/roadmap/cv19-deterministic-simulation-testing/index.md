...
level: Value
status: Active
status_reason: CV19.DS1 through DS3 are Done and DS4 now has bounded ordinary/scheduled campaign tiers; failure operations and complementary production-boundary qualification remain active
updated: 2026-08-19
related:
...
2. [CV19.DS2 — Deterministic event and fault harness](cv19-ds2-deterministic-event-and-fault-harness.md)
   ...
   pair of separately identified real Absurd/PostgreSQL profiles proves the
   stronger joined-begin shape: either a refused begin commit or a failing task
   ...
   semantic begin and task without another preparation. A fourth refuses the
   ...
   fresh-load recollection plus one `FiringFailed`. A seventh loses the accepted
   `ActivityFailed` transaction's acknowledgement and proves fresh load appends
   one `FiringFailed` without provider recollection. An eighth refuses the later
   projection commit and proves the same fresh-load projection without another
   Worker completion or handler preparation. A ninth loses the accepted
   projection acknowledgement and proves fresh load reconstructs the converged
   state without another projection. A tenth refuses the canonical
   `ScopeReset` commit and proves fresh load retains generation 1 and accepts
   its still-authoritative Worker completion exactly once. An eleventh commits
   `ScopeReset` but loses that transaction's acknowledgement before
   cancellation begins, then proves fresh-load reconstruction of generation 2,
   one repaired tombstone, and stale-Worker fencing. A twelfth commits
   `ScopeReset`, refuses the separate real Absurd cancellation-tombstone
   commit, then proves fresh-load repair and stale-Worker fencing without a
   ...
   fresh load does not append another or recollect provider custody. A
   ...
   another creation transaction. A thirtieth refuses a projection containing
...
4. [CV19.DS4 — Campaign and production-boundary qualification](cv19-ds4-campaign-and-production-boundary-qualification.md)
   is Active. Its first Technical Story keeps ordinary generation in pytest and
   adds one bounded, seed-addressed scheduled cohort with payload-free semantic
   reporting. Remaining work rehearses failure retention/promotion and proves
   how simulation composes with real storage, transport, and process-kill tests
