...
A seventh profile proves accepted failed-terminal acknowledgement loss:

1. claim the joined task and report one non-retryable `ActivityFailure` through
   a real public `AbsurdWorkerDispatch`, then let production commit
   `ActivityFailed`;
2. lose the failed-terminal transaction's commit acknowledgement and
   independently observe failed custody, one canonical failure, no
   `FiringFailed`, and a poisoned Engine;
3. revoke and abruptly drop that generation, then load through public
   `load_engine`;
4. append exactly one `FiringFailed` without provider recollection, another
   Worker failure, task, or handler `prepare`; and
5. replay the retained 12 operations and 21 journal entries, including 9
   independent checker evaluations, exactly, ending in `quarantined`.

A companion pair proves the later failed-firing projection transaction:

1. begin one Activity, report one non-retryable failure through a real public
   `AbsurdWorkerDispatch`, and let production commit `ActivityFailed`;
2. either refuse the separate `FiringFailed` transaction before acceptance or
   commit it and lose the acknowledgement at the connection boundary;
3. independently observe failed provider custody, one accepted terminal
   failure, and respectively no accepted or exactly one accepted failed-firing
   projection while the Engine poisons;
4. revoke and abruptly drop that generation, then load through public
   `load_engine`;
5. respectively append `FiringFailed` exactly once or reconstruct it without
   another projection, Worker failure, provider recollection, task, or handler
   `prepare`; and
6. replay each retained 12-operation, 21-entry journal including 9 independent
   checker evaluations exactly, ending in `quarantined`.

An eighth separately identified real-provider profile proves the later
...
A ninth separately identified real-provider profile proves the accepted side
...
A tenth separately identified real-provider profile proves the canonical
reset transaction boundary before operational cancellation:

1. open lifecycle generation 1, begin one Activity, and claim its real Absurd
   task through public Engine and Worker doors;
2. refuse the transaction containing `ScopeReset` before PostgreSQL accepts it,
   then independently observe no canonical reset or cancellation attempt, the
   still-running task, and a poisoned Engine generation;
3. revoke and abruptly drop that generation while the external Worker remains
   alive, then load through public `load_engine`;
4. reconstruct generation 1 and the same in-flight invocation without another
   handler `prepare`;
5. accept the original Worker's completion as authoritative rather than stale,
   and append exactly one `ActivityCompleted` plus one projection without a
   reset or cancellation tombstone; and
6. replay the retained 21 operations and 34 journal entries, including 13
   independent checker evaluations, exactly, ending in `quiescent`.

An eleventh separately identified real-provider profile proves the accepted
side of the canonical reset transaction before operational cancellation:

1. open lifecycle generation 1, begin one Activity, and claim its real Absurd
   task through public Engine and Worker doors;
2. let production commit `ScopeReset`, then lose that transaction's
   acknowledgement before the separate cancellation transaction begins;
3. independently observe canonical generation 2, the still-running task, no
   cancellation attempt, and a poisoned Engine generation;
4. revoke and abruptly drop that generation while the external Worker remains
   alive, then load through public `load_engine`;
5. repair exactly one cancellation tombstone without another reset, task, or
   handler `prepare`, and prove the old Worker's completion is refused as stale
   without an `ActivityCompleted` or business projection; and
6. replay the retained 19 operations and 31 journal entries, including 12
   independent checker evaluations, exactly, ending in `quiescent`.

A twelfth separately identified real-provider profile proves the lifecycle
...
A thirteenth separately identified real-provider profile proves the accepted side
...
1. open lifecycle generation 1, reset to generation 2, and submit one
   identified delivery through public `Engine.deliver` against the stale exact
   generation;
2. either refuse `ScopedDeliveryDropped` before PostgreSQL acceptance or commit
   it and lose the acknowledgement;
3. independently compare the accepted/refused transaction with detached
   lifecycle, identity, payload, scope, and disposition facts;
4. revoke and abruptly drop the poisoned generation, then load through public
   `load_engine`;
5. respectively accept the drop exactly once or reconstruct it, then prove an
   exact redelivery returns the prior `dropped` acknowledgement without another
   History transaction; and
6. replay the retained 13/11 operations and 22/19 journal entries, including
   9/8 independent checker evaluations, exactly, ending in `external_wait`.

The paired future-scope route proves quarantine authority at the same public
delivery boundary:

1. open lifecycle generation 1 and submit one identified delivery through
   public `Engine.deliver` against the future exact generation 2;
2. either refuse `ScopedDeliveryQuarantined` before PostgreSQL acceptance or
   commit it and lose the acknowledgement;
3. independently compare the accepted/refused transaction with detached
   lifecycle, identity, payload, scope, and disposition facts;
4. revoke and abruptly drop the poisoned generation, then load through public
   `load_engine`;
5. respectively accept the quarantine exactly once or reconstruct it, then
   prove exact redelivery returns the prior `quarantined` acknowledgement
   without another History transaction; and
6. replay the retained 12/10 operations and 20/17 journal entries, including
   8/7 independent checker evaluations, exactly, ending in `external_wait`.
...
Two additional real-provider profiles prove canonical scope creation. One
refuses `ScopeOpened` before acceptance, reconstructs absence, and retries
generation 1 exactly once. The other loses acknowledgement after acceptance,
then fresh public load reconstructs `draft:1` without another open or
generation 2. The retained routes replay 9/7 operations and 15/12 journal
entries, including 6/5 independent checker evaluations, ending in
`external_wait`.

Two additional profiles distinguish source-registration initialization from
the already-qualified token-bearing creation batch. A source-transition net
creates `InstanceCreated + DeliveryRegistrationOpened` atomically. One profile
refuses that transaction, proves detached absence after abrupt drop/load, and
retries creation exactly once. The other commits it but loses acknowledgement,
then public load reconstructs the exact identity and one armed default source
registration without another transaction. The retained routes replay 9/7
operations and 16/13 journal entries, including 6/5 independent checker
...
| 6. Named cuts before/after durable acceptance | Partial: paired pre-commit refusal/post-commit acknowledgement-loss History cuts, paired token-bearing and source-registration initial creation, split Dispatch refusal after a durable request and lifecycle fence, joined begin+spawn commit refusal, task-spawn rollback, accepted-begin acknowledgement loss, paired identified/stale/future-scope delivery, completed-terminal, failed-terminal, successful/failed projection, canonical open/reset, and terminal-close commit refusal/acknowledgement loss, paired refused/accepted-but-unacknowledged post-reset cancellation-tombstone recovery, delayed external completion, lifecycle and timer reconstruction, zero- and nonzero-backoff LocalDispatch retry reconstruction/exhaustion, and provider-terminal custody before Engine collection are proved; the broader transaction matrix remains. |
...
| 10. Repository gates | Met: focused DST and project tests pass, and the current full and release gates pass all 2,370 tests in both release orders with 17 expected serial qualification deselections. |
...
- `UV_FROZEN=1 uv run pytest -q tests/dst` — 140 passed.
- `UV_FROZEN=1 uv run pytest -q tests/dst tests/project` — 169 passed.
...
- `UV_FROZEN=1 uv run pytest -q
  tests/dst/test_joined_creation_world.py::test_retained_joined_creation_refusal_replays_without_the_authored_scenario`
  returned `pass` / `external_wait`, 9 operations, 16 journal entries including
  6 checker evaluations, and digest
  `sha256:f9a40de727afd825a844fb5657d851fc5b49a7e509c424cfc75c9c75052fbe6a`.
- `UV_FROZEN=1 uv run pytest -q
  tests/dst/test_joined_creation_world.py::test_retained_joined_creation_ack_loss_replays_without_the_authored_scenario`
  returned `pass` / `external_wait`, 7 operations, 13 journal entries including
  5 checker evaluations, and digest
  `sha256:cc2123263ec933f7fb57c21c05a19f2a44dcb413556bb239ee1816596bd80836`.
- `UV_FROZEN=1 uv run pytest -q
  tests/dst/test_joined_creation_world.py::test_retained_joined_source_creation_refusal_replays_without_the_authored_scenario`
  returned `pass` / `external_wait`, 9 operations, 16 journal entries including
  6 checker evaluations, and digest
  `sha256:fd75f35182dff7f1459ccb62ced71bc1ef3820f275987623ee9e5e03d2f95891`.
- `UV_FROZEN=1 uv run pytest -q
  tests/dst/test_joined_creation_world.py::test_retained_joined_source_creation_ack_loss_replays_without_the_authored_scenario`
  returned `pass` / `external_wait`, 7 operations, 13 journal entries including
  5 checker evaluations, and digest
...
- `UV_FROZEN=1 uv run pytest -q
  tests/dst/test_joined_world.py::test_retained_joined_failure_ack_loss_replays_without_the_authored_scenario`
  returned `pass` / `quarantined`, 12 operations, 21 journal entries including
  9 checker evaluations, and digest
...
- `scripts/check full` — 2,370 passed.
- `scripts/check release` — 2,370 passed in both the four-worker and fixed-order
