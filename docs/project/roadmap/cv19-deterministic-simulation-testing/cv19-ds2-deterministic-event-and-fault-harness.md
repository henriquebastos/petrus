...
status: Active
status_reason: Split Dispatch refusals and real joined begin plus paired terminal, projection, and lifecycle-cancellation refusal/acknowledgement-loss recovery now replay through crash with exact repair; DS2 remains active for the broader transaction cut matrix
updated: 2026-08-18
related:
  - index.md
...
A sixth separately identified real-provider profile proves the later
...
An eighth separately identified real-provider profile proves the lifecycle
...
| 2. Imperative story emits replay data | Met by the executable profiles under `tests/dst/` and retained projection, pre/post-commit History, split Dispatch request/cancellation refusal, joined begin commit/task-spawn rollback and post-commit acknowledgement loss, paired joined terminal refusal/acknowledgement-loss and projection recovery, joined lifecycle cancellation repair, ingress-redelivery, delayed-terminal, lifecycle-race, timer-recovery, zero/delayed retry-exhaustion, terminal-recollection, and resource-bound fixtures. |
| 3. Same-interpreter replay agreement | Met for all twenty-three public-Engine/provider crash/recovery stories plus the three retained generic failures, including exact operations, resource/checker observations, dispositions, and journal digests. |
...
| 10. Repository gates | Met: focused DST and project tests pass, and the current full and release gates pass all 2,318 tests in both release orders with 17 expected serial qualification deselections. |
...
- `UV_FROZEN=1 uv run pytest -q tests/dst` — 131 passed.
- `UV_FROZEN=1 uv run pytest -q tests/dst tests/project` — 160 passed.
...
- `scripts/check full` — 2,318 passed.
- `scripts/check release` — 2,318 passed in both the four-worker and fixed-order
