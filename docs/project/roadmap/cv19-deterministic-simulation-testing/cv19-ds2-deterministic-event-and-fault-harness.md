...
Two further profiles qualify handler-authored registration effects inside the
projection boundary. The deterministic handler closes `source/default` and
opens `source/replacement` in the same transaction as `TokensProduced` and
`FiringCompleted`. Refusal retains only the default registration until fresh
load retries the frozen projection once; acknowledgement loss reconstructs
only the replacement and performs no second projection. Both retained routes
...
- `UV_FROZEN=1 uv run pytest -q
  tests/dst/test_joined_registration_world.py::test_retained_joined_registration_projection_refusal_replays_without_authored_scenario`
  returned `pass` / `external_wait`, 12 operations, 21 journal entries including
  9 checker evaluations, and digest
  `sha256:7c0d1b65029dd0ac9808b912e154c1df516ac23248efead9b73ebc73a97f906b`.
- `UV_FROZEN=1 uv run pytest -q
  tests/dst/test_joined_registration_world.py::test_retained_joined_registration_projection_ack_loss_replays_without_authored_scenario`
  returned `pass` / `external_wait`, 12 operations, 21 journal entries including
  9 checker evaluations, and digest
